...
pub fn get_tilemap_coord_transform(coord: &IVec2, size: &UVec2, tile_size: &Vec2, y: f32) -> Transform {
    let translation = chunk_coord_to_world_pos(coord, size, tile_size).extend(y).xzy();
...
/// `coord` in world space.
pub fn get_tile_coord_transform(coord: &IVec2, size: &UVec2, tile_size: &Vec2, y: f32) -> Transform {
    let translation = tile_coord_to_world_offset(coord, size, tile_size).extend(y).xzy();

    return Transform::from_translation(translation);
}

pub fn tile_coord_to_world_offset(coord: &IVec2, size: &UVec2, tile_size: &Vec2) -> Vec2 {
    let offset = coord.as_vec2() * *tile_size - size.as_vec2() * *tile_size / 2.0 + *tile_size / 2.0;
